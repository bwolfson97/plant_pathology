...
__all__ = ['load_data', 'create_folds', 'kaggle_submit_command', 'average_preds', 'get_averaged_preds']
...
# Cell
def create_folds(path: Path, print_stats: bool = False, save: bool = False) -> pd.DataFrame:
    """Returns train CSV at `path` with 5-fold CV splits added."""
    # Load train CSV
    df = pd.read_csv(path)
    ...
    if print_stats:
        print(df.groupby("fold").describe())
